...
import { STATUS_FARG, type ATAStatus } from '@/sections/ata/types'
import { useAppConfig } from '@/context/AppConfig'

const STATUSAR: ATAStatus[] = ['Utkast', 'Skickad', 'Godkänd', 'Avvisad']
...
export function ATAPanel() {
  ...
  useEffect(() => {
    window.api.invoke('db:ata-nummer:peek').then((n) => {
      setCurrent(n as number)
      setInput(String(n as number))
    })
  }, [])

  useEffect(() => { setVillkorDraft(config?.ata_std_villkor ?? '') }, [config?.ata_std_villkor])

  async function saveVillkor() {
    if (villkorDraft === (config?.ata_std_villkor ?? '')) return
    await updateConfig({ ata_std_villkor: villkorDraft })
  ...
  async function handleSet() {
    ...
    try {
      const next = await window.api.invoke('db:ata-nummer:set', val) as number
  ...
  async function handleReset() {
    ...
    try {
      const next = await window.api.invoke('db:ata-nummer:set', 1) as number
      setCurrent(next); setSaved(true); setTimeout(() => setSaved(false), 2500)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Fel')
    } finally { setSaving(false) }
    ...
    <div className="flex h-full min-h-0">
      <div className="flex flex-col flex-1 border-r border-border overflow-auto">
        <div className="px-8 py-6 border-b border-border">
          <p className="text-[11px] uppercase tracking-widest text-muted mb-4">ÄTA-numrering</p>
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-xs text-subtle">
              Nästa: <span className="font-mono text-fg">ÄTA-{current != null ? String(current).padStart(4, '0') : '…'}</span>
            </span>
            <input
          ...
          <p className="mt-4 text-xs text-muted">ÄTA numreras automatiskt som ÄTA-NNNN. Sätt nästa nummer manuellt vid behov (t.ex. vid årsskifte).</p>
        </div>

        <div className="px-8 py-6 border-b border-border">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[11px] uppercase tracking-widest text-muted">Standardvillkor för ÄTA (global)</p>
          ...
          <textarea
            ...
            onBlur={saveVillkor}
            rows={6}
            placeholder="t.ex. Detta arbete utgör ett tilläggsarbete utöver ursprunglig offert/avtal och faktureras separat. Arbetet påbörjas först efter att kunden har godkänt och signerat denna ÄTA."
            className="input resize-y leading-relaxed w-full"
          />
          <p className="mt-2 text-[11px] text-subtle leading-relaxed">
            Används som mall när en ny ÄTA skapas. Kaskad: <span className="text-muted">ÄTA-input</span> →{' '}
            <span className="text-muted">kundens egna ÄTA-villkor</span> →{' '}
            <span className="text-muted">denna globala mall</span>. Snapshot — redan skapade ÄTA-arbeten påverkas inte.
          </p>
        </div>
      </div>
      ...
      <div className="w-72 flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-auto">
          ...
          <p className="px-6 py-4 text-[11px] text-subtle leading-relaxed">
            Statusarna är fasta för ÄTA: Utkast → Skickad → Godkänd / Avvisad. De kan inte ändras eftersom de är kopplade till signeringsflödet.
          </p>
        </div>
      </div>
    </div>

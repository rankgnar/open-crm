...
const MATERIAL_PRESETS: MaterialPreset[] = [
  { name: 'Betongpannor',  unit: 'st', coveragePerM2: 11,   unitPrice: 0 },
  { name: 'Råspont 22×95', unit: 'm',  coveragePerM2: 10.5, unitPrice: 0 },
  { name: 'Underlagstak',  unit: 'm²', coveragePerM2: 1.1,  unitPrice: 0 },
  { name: 'Nocklist',      unit: 'm',  coveragePerM2: 0.06, unitPrice: 0 },
  { name: 'Nockpannor',    unit: 'st', coveragePerM2: 0.5,  unitPrice: 0 },
]
...
export function TakKalkylator({ input, avdragPresets, takTyper, onUpdate }: Props) {
  const addSurface = (shape: 'rectangle' | 'triangle') => {
    onUpdate({
      ...input,
      surfaces: [
        ...input.surfaces,
        { id: crypto.randomUUID(), description: '', shape, width: 0, height: 0, slopeFactor: 1.0 },
      ],
    })
  }
  ...
  const addPreset = (preset: MaterialPreset) => {
    onUpdate({ ...input, materials: [...input.materials, { id: crypto.randomUUID(), ...preset }] })
  }

  const addCustomMaterial = () => {
    onUpdate({ ...input, materials: [...input.materials, { id: crypto.randomUUID(), name: '', unit: 'm²', coveragePerM2: 1, unitPrice: 0 }] })
  }
  ...
  return (
    <div className="flex flex-col divide-y divide-border">
        ...
        {input.surfaces.length > 0 && (
          <div className="flex flex-col gap-2 mb-4">
            {input.surfaces.map(row => (
              <SurfaceCard key={row.id} row={row} takTyper={takTyper} onChange={updated => updateSurface(row.id, updated)} onDuplicate={() => duplicateSurface(row.id)} onRemove={() => removeSurface(row.id)} />
            ))}
          </div>
        )}
      ...
      {/* === Avdrag === */}
      <div className="px-6 py-6">
        <SectionHeader title="Öppningar — takfönster & genomföringar" sub={totalDeduction > 0 ? `−${totalDeduction.toLocaleString('sv-SE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m²` : undefined} />
      ...
      <div className="px-6 py-6">
        ...
        <div className="flex flex-wrap gap-2">
          {MATERIAL_PRESETS.map(p => (
            <button key={p.name} onClick={() => addPreset(p)} className="text-xs px-2.5 py-1 rounded border border-border text-muted hover:text-fg hover:border-hover transition-colors">+ {p.name}</button>
          ))}
      ...
      <div className="px-6 py-6">
        ...
        <div className="flex items-end gap-6 flex-wrap">
          ...
          {netArea > 0 && (
            <div className="mb-0.5 text-sm text-muted">= <span className="text-fg font-medium">{(netArea * input.laborHoursPerM2).toLocaleString('sv-SE', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}</span> timmar totalt</div>
          )}
        </div>
      </div>
    </div>

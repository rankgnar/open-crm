...
import type { CreateOrderInput, CreateOrderRadInput, FasTreeNode } from './types'
...
interface RadDraft extends CreateOrderRadInput {
  key: string
}
...
export function OrderForm({ projekt, onSubmit, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    // Pre-fill villkor with cascade: kund's template → global default. Never
    // overwrite what the user has typed.
    if (!villkorTouched && valda?.kund_id) {
      window.api.invoke('db:kunder:get', valda.kund_id)
        .then((k) => {
          const fromKund = ((k as Kund).order_std_villkor ?? '').trim()
          setVillkor(fromKund || (config?.order_std_villkor ?? ''))
        })
        .catch(() => setVillkor(config?.order_std_villkor ?? ''))
    }
  }, [projekt_id])
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-sidebar shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-fg text-sm font-medium">Ny order</span>
        </div>
          ...
          <div className="grid grid-cols-3 gap-x-8 gap-y-5">
            ...
            <div className="flex flex-col gap-1.5 col-span-3">
              <label className="text-xs text-muted">
                Villkor
                {!villkorTouched && projekt_id && <span className="ml-2 text-subtle font-normal">(auto-ifylld från kundens standardvillkor)</span>}
              </label>
              <textarea
                className="input min-h-24"
                value={villkor}
                onChange={(e) => { setVillkor(e.target.value); setVillkorTouched(true) }}
                placeholder="Specifika villkor för denna order..."
              />
            </div>
          </div>
        </div>

...
import type { ATAStatus } from './types'
import { STATUS_FARG } from './types'

const EDITABLE_STATUSAR: ATAStatus[] = ['Utkast', 'Skickad', 'Avvisad']

interface Props {
  status: ATAStatus
  onChange: (next: ATAStatus) => Promise<void> | void
  /** When true, click on Godkänd chip triggers onRequestUnlock instead of opening dropdown */
  onRequestUnlock?: () => void
}

export function ATAStatusPicker({ status, onChange, onRequestUnlock }: Props) {
  const [open, setOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
  const farg = STATUS_FARG[status]

  function handleSelect(e: React.MouseEvent, next: ATAStatus) {
    e.stopPropagation()
    setOpen(false)

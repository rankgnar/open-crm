-- Bug: customer confirmation went to kunder.email (potentially stale) instead
-- of the specific email the admin typed when sending the link. Fix: always
-- use signatur_lankar.kund_email as the destination — that's the address the
-- link was sent to, and it's what the customer expects.
...
BEGIN
  ...
  IF v_log_aktiv IS TRUE THEN
    -- Customer email: ALWAYS the address the admin typed when sending the link.

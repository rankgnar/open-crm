-- The view-tracking rewrite in 20260503100000_track_forslag_views.sql
-- accidentally dropped three fields from the get_signing_doc RETURN
-- payload: andring_begard_at, andring_historik, revisioner_historik.
-- The sign portal (open-crm-sign) accesses state.payload.andring_historik
-- unconditionally on render, so the missing field crashed React with
-- "Cannot read properties of undefined (reading 'length')" — every
-- signing link opened to a blank page.
--
-- This migration replays the 20260503100000 function body verbatim and
-- adds the three fields back to the RETURN object.

-- Allow per-link klientportal auto-invite for 'fritt' documents.
--
-- Today the auto-invite block in submit_signature() (last set in
-- 20260430130000) only fires for `dokument_typ = 'forslag'` and respects
-- the global toggle `app_installningar.kund_portal_auto_invite`. That
-- means a customer who signs a free-form PDF via Signera never gets
-- klientportal access — even when the admin wants exactly that, e.g.
-- when uploading an offer that was negotiated outside the CRM.
--
-- This migration:
--   1. Adds `auto_invite_kund_portal BOOLEAN NOT NULL DEFAULT FALSE` to
--      `signatur_lankar` so each link can opt-in independently.
--   2. Replaces submit_signature() with the same body, plus a new branch
--      that fires the invite for 'fritt' when the link's flag is true.
--
-- The 'forslag' branch keeps using the global toggle exactly as before.

BEGIN;

ALTER TABLE signatur_lankar
  ADD COLUMN IF NOT EXISTS auto_invite_kund_portal BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN signatur_lankar.auto_invite_kund_portal IS
  'Per-link opt-in: when a fritt dokument is signed, queue a klientportal welcome invite for the kund. Forslag still uses app_installningar.kund_portal_auto_invite.';
...
BEGIN
  ...
  -- Klientportal auto-invite. Two paths:
  --   * forslag → respects the global toggle app_installningar.kund_portal_auto_invite.
  --   * fritt   → respects the per-link flag signatur_lankar.auto_invite_kund_portal,
  --               so the admin opts-in case by case from the Signera modal.
  -- The Electron worker picks up rows from kund_portal_invite_queue, creates/finds
  -- the auth user, and queues the welcome email with a real action_link.
  -- Idempotent at the worker level via kund_users (auth_user_id, kund_id) UNIQUE.

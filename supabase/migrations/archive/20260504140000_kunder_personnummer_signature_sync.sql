-- Capture the customer's personnummer through the signing flow.
--
-- The signing portal already collects a personnummer and stores it on
-- the link (signatur_lankar.signerad_personnummer). This migration:
--
--   1. Adds `personnummer text` to kunder so the CRM can keep it on the
--      customer record itself, like medsokande_personnummer already is.
--   2. Wires submit_signature() to sync the value back to kunder when
--      the customer signs:
--        * kund had no personnummer       → write the signed one
--        * kund's personnummer matches    → no-op
--        * kund's personnummer differs    → log a projekt anteckning
--          flagging the mismatch (don't auto-overwrite — the admin
--          decides what's correct).
--
-- Comparison strips non-digits and matches on the last 10 digits, so
-- the 10-vs-12-digit and dash/no-dash variants all behave the same.

BEGIN;

ALTER TABLE kunder ADD COLUMN IF NOT EXISTS personnummer TEXT;

-- ──────────────────────────────────────────────────────────────────────────
-- Helpers
-- ──────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION normalize_personnummer(p TEXT)
RETURNS TEXT
...
-- Reformat a normalized personnummer (10 or 12 digits) into the canonical
-- visible form with the dash. Anything else returned as-is.
CREATE OR REPLACE FUNCTION format_personnummer(p TEXT)
RETURNS TEXT
LANGUAGE sql IMMUTABLE
AS $$
...
-- ──────────────────────────────────────────────────────────────────────────
-- submit_signature: sync personnummer back to the kund record
-- ──────────────────────────────────────────────────────────────────────────

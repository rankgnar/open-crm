-- Refactor submit_signature so the kund/admin confirmation emails come from
-- editable epost_mallar (looked up via system_kod) instead of hardcoded HTML
-- inside the function. Falls back to the previous inline strings if the mall
-- is missing — never breaks the signing flow.
...
BEGIN
  ...
  IF v_log_aktiv IS TRUE THEN
    ...
    -- Default alias fallback (used if a mall has no alias_id set)
    SELECT id INTO v_alias_id FROM epost_alias WHERE aktiv ORDER BY standard DESC NULLS LAST, sortering LIMIT 1;

    -- Assemble the variable bag for both confirmation mallar
    v_doc_typ_label     := CASE WHEN v_link.dokument_typ = 'forslag' THEN 'offert' ELSE 'order' END;
    v_doc_typ_label_def := CASE WHEN v_link.dokument_typ = 'forslag' THEN 'offerten' ELSE 'ordern' END;

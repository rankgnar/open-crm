-- Track every time a customer opens a signing link (not just the first time).
-- Existing `oppnad_at` keeps the first-view timestamp. We add:
--   * view_count      — total opens (increments on every get_signing_doc call)
--   * last_oppnad_at  — most recent open timestamp
--
-- For forslag links we also emit an aktivitetslogg entry on the project
-- timeline so the admin can see engagement. Throttled to one entry per
-- 30-minute window to avoid spam from page refreshes inside one session.

BEGIN;

ALTER TABLE signatur_lankar
  ADD COLUMN IF NOT EXISTS view_count     INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS last_oppnad_at TIMESTAMPTZ;

INSERT INTO aktivitetslogg_installningar (handelse, etikett, aktiv, kategori) VALUES
  ('kund_oppnade_forslag', 'Kund öppnade förslag', true, 'signatur')

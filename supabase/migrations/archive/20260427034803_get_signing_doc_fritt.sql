-- Extend get_signing_doc to resolve the 'fritt' branch — the portal needs
-- a doc payload (titel + projekt + kund) and an empty lines array. The
-- signed PDF is rendered from document_pdf_url (the original upload), so
-- no derived structure is needed.
